...
import { S as Sidebar_provider, a as Sidebar_inset, b as Sidebar, c as Sidebar_header, d as Sidebar_content, e as Sidebar_trigger, g as Sidebar_menu, h as Sidebar_menu_item, f as Sidebar_group } from './sidebar-trigger-CyHJa3-O.js';
...
import { c as currentOrg, o as orgs, a as PLAN, i as isOrgAdmin, b as currentOrgPath, d as isFreePlan, f as PLAN_NAMES, m as mergeAccountOrgFromServer } from './org-t71AQfHV.js';
import { a as store_get, u as unsubscribe_stores, z as store_mutate, n as escape_html, f as ensure_array_like, c as bind_props, d as attr, g as attributes, e as stringify, b as spread_props, t as props_id, k as derived } from './index2-Eg0dVEDW.js';
import { S as Separator } from './separator-C8wJJtqC.js';
import { R as Root$2, P as Popover_trigger, a as Popover_content } from './index9-C_LBahdk.js';
import { M as Menu, S as Search, R as Rocket, B as Breadcrumb, a as Breadcrumb_list, b as Breadcrumb_item, c as Breadcrumb_separator, d as Breadcrumb_ellipsis, D as Drawer_trigger, g as Breadcrumb_link, h as Breadcrumb_page, j as Dropdown_menu_label, C as Chevrons_up_down, e as Drawer_description, f as Drawer_footer, i as Drawer_close } from './menu-CMJe-Q2d.js';
import { a as createBitsAttrs, M as MediaQuery, n as noop, e as boxWith, C as Context, w as watch, d as createId, m as mergeProps, b as attachRef, $ as isTouch, k as isFocusVisible, l as getDataOpenClosed, h as boolToStr, j as isElement } from './create-id-D7gdjJzW.js';
...
import { g as getOrgNavigationGroups, a as getOrgNavigationItems } from './org-navigation-B6F9Gh6m.js';
...
import { s as setTheme } from './theme-jlexj1Qp.js';
...
import { s as setupProgressApi, c as calculateProgress } from './setup-progress.svelte-DpnO-FI3.js';
import { o as on } from './events-BzIOqpZK.js';
import { a as PresenceManager, t as getTabbableCandidates, c as afterSleep, P as Portal } from './scroll-lock-CqTUXsNd.js';
import { S as SafePolygon } from './safe-polygon.svelte-D5_q4acK.js';
import { D as DOMContext } from './dom-context.svelte-BWqx6Zqn.js';
import { F as Floating_layer, P as Popper_layer_force_mount, a as Popper_layer, g as getFloatingContentCSSVars } from './popper-layer-force-mount-H2EBI_WO.js';
import { S as Setup_progress, B as Badge_check } from './badge-check-BUqy5Uq4.js';
import { p as profile } from './user-Ahqcoe6u.js';
import { F as Floating_layer_anchor } from './floating-layer-anchor-CBXkOzVQ.js';
import { M as Mounted } from './mounted-B958bjT3.js';
...
import { g as globalStore } from './app-CdpE9djd.js';
import './index4-CY3O4yvX.js';
import './submission-CIIjGYOS.js';
import './constants-C43vhkR3.js';
import 'posthog-js';
import { T as TENANT_ROOT_DOMAIN, B as BRAND_ROOT_DOMAIN } from './domains-D9J1UytB.js';
...
import { g as generateSitename, o as openUpgradeModal } from './org2-B8QQInlf.js';
...
import './attachment-DuGjlKEs.js';
import './course-CF1uUloU.js';
import './goal-C_iaeEkX.js';
import './home-C9RNcUNP.js';
import './bot-f-4w3Uyd.js';
import './people-DwBXfNtv.js';
...
function generateBreadcrumbs(pathname, searchParams, navItems, currentOrgPath2, t2, pageData) {
  const breadcrumbs = [];
  const pathWithQuery = pathname + searchParams;
  const relativePath = pathname.replace(currentOrgPath2, "").replace(/^\//, "") || "";
  ...
  if (!matchedNavItem) {
    for (const item of navItems) {
      const fullItemPath = `${currentOrgPath2}${item.path}`;
  ...
  if (matchedNavItem) {
    const parentCrumbPath = matchedNavItem.useHashUrl && matchedNavItem.items?.length ? matchedNavItem.items[0].path : matchedNavItem.path;
    const href = matchedNavItem.useHashUrl ? `${currentOrgPath2}${parentCrumbPath}` : matchedNavItem.url;
    breadcrumbs.push({
  ...
  if (matchedNavItem?.items && !searchParams) {
    const parentBase = `${currentOrgPath2}${matchedNavItem.path}`.replace(/\/$/, "");
  ...
  if (pathSegments.length > 1 && matchedNavItem) {
    ...
    if (matchedNavItem.nestedRoutes) {
      ...
      if (nestedRoute) {
        breadcrumbs.push({
          ...
          href: `${currentOrgPath2}${matchedNavItem.path}/${nestedRoute.path}`
        });
        return breadcrumbs;
      }
    }
    if (matchedNavItem.supportsDynamicSegment && secondSegment) {
      ...
      if (!isStaticRoute && pageData?.breadcrumb) {
        breadcrumbs.push({
          label: t2(pageData.breadcrumb),
          href: `${currentOrgPath2}${matchedNavItem.path}/${secondSegment}`
        });
      }
      return breadcrumbs;
    }
...
function Org_switcher($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    const sidebar = useSidebar();
    let { variant = "sidebar" } = $$props;
    function rootIdFor(org) {
      return org.parentOrganizationId ?? org.id;
    }
    const accountRootId = rootIdFor(store_get($$store_subs ??= {}, "$currentOrg", currentOrg));
    const accountWorkspaces = store_get($$store_subs ??= {}, "$orgs", orgs).filter((org) => rootIdFor(org) === accountRootId);
    const otherOrgs = store_get($$store_subs ??= {}, "$orgs", orgs).filter((org) => rootIdFor(org) !== accountRootId);
    function onClick(org) {
      if (org.id === store_get($$store_subs ??= {}, "$currentOrg", currentOrg).id) return;
      localStorage.setItem("classroomio_org_sitename", org.siteName);
      currentOrg.set(mergeAccountOrgFromServer(org));
      setTheme(org.theme);
      window.location.href = store_get($$store_subs ??= {}, "$currentOrgPath", currentOrgPath);
    }
    {
      $$renderer2.push("<!--[!-->");
          ...
          children: ($$renderer3) => {
            $$renderer3.push(`<!---->`);
            {
              let child = function($$renderer4, { props }) {
                $$renderer4.push(`<!---->`);
                Breadcrumb_link($$renderer4, spread_props([
                  props,
                  {
                    href: "##",
                    ...
                    children: ($$renderer5) => {
                      if (store_get($$store_subs ??= {}, "$currentOrg", currentOrg).name) {
                        ...
                        $$renderer5.push(`<!----> <span class="hidden truncate text-sm font-medium md:block">${escape_html(store_get($$store_subs ??= {}, "$currentOrg", currentOrg).name)}</span> `);
                        Chevron_down($$renderer5, { class: "ml-auto hidden size-4 md:block" });
                        $$renderer5.push(`<!---->`);
                      } else {
                        $$renderer5.push("<!--[!-->");
            ...
            Dropdown_menu_content($$renderer3, {
              ...
              children: ($$renderer4) => {
                ...
                Dropdown_menu_label($$renderer4, {
                  class: "ui:text-muted-foreground ui:text-xs",
                  children: ($$renderer5) => {
                    $$renderer5.push(`<!---->${escape_html(store_get($$store_subs ??= {}, "$t", t)("account.switcher.your_account"))}`);
                  },
                  $$slots: { default: true }
                });
                $$renderer4.push(`<!----> <!--[-->`);
                const each_array = ensure_array_like(accountWorkspaces);
                for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
                  let org = each_array[$$index];
                  $$renderer4.push(`<!---->`);
                  Dropdown_menu_item($$renderer4, {
                    onSelect: () => onClick(org),
                    class: "gap-2 p-2",
                    children: ($$renderer5) => {
                      $$renderer5.push(`<!---->`);
                      Avatar($$renderer5, {
                        ...
                        children: ($$renderer6) => {
                          $$renderer6.push(`<!---->`);
                          Avatar_image($$renderer6, { src: org.avatarUrl, alt: org.name });
                          $$renderer6.push(`<!----> <!---->`);
                          Avatar_fallback($$renderer6, {
                            class: "rounded-lg",
                            children: ($$renderer7) => {
                              $$renderer7.push(`<!---->${escape_html(shortenName(org.name))}`);
                            },
                            $$slots: { default: true }
                          });
                          $$renderer6.push(`<!---->`);
                ...
                if (otherOrgs.length > 0) {
                  ...
                  Dropdown_menu_label($$renderer4, {
                    class: "ui:text-muted-foreground ui:text-xs",
                    children: ($$renderer5) => {
                      $$renderer5.push(`<!---->${escape_html(store_get($$store_subs ??= {}, "$t", t)("account.switcher.other_organizations"))}`);
                    },
                    $$slots: { default: true }
                  });
                  $$renderer4.push(`<!----> <!--[-->`);
                  const each_array_1 = ensure_array_like(otherOrgs);
                ...
                $$renderer4.push(`<!----> <!---->`);
                Dropdown_menu_item($$renderer4, {
                  class: "cursor-not-allowed gap-2 p-2 opacity-50",
                  children: ($$renderer5) => {
                    $$renderer5.push(`<div class="flex size-6 items-center justify-center rounded-md border bg-transparent">`);
                    Plus($$renderer5, { class: "size-4" });
                    $$renderer5.push(`<!----></div> <div class="ui:text-muted-foreground ui:font-normal">Add Organization</div> `);
                    Coming_soon($$renderer5, {});
                    $$renderer5.push(`<!---->`);
                  },
                  $$slots: { default: true }
                });
                          ...
                          props,
                          {
                            ...
                            children: ($$renderer7) => {
                              if (store_get($$store_subs ??= {}, "$currentOrg", currentOrg).name) {
                                ...
                                $$renderer7.push(`<!---->`);
                                Avatar($$renderer7, {
                                  class: "ui:flex ui:size-8 ui:items-center ui:justify-center ui:rounded-lg",
                                  children: ($$renderer8) => {
                                    $$renderer8.push(`<!---->`);
                                    ...
                                    Avatar_fallback($$renderer8, {
                                      class: "rounded-lg",
                                      children: ($$renderer9) => {
                                        $$renderer9.push(`<!---->${escape_html(shortenName(store_get($$store_subs ??= {}, "$currentOrg", currentOrg).name))}`);
                                      },
                                      $$slots: { default: true }
                                    });
                                    $$renderer8.push(`<!---->`);
                                ...
                                $$renderer7.push(`<!----> <div class="grid flex-1 text-left text-sm leading-tight"><span class="truncate font-normal">${escape_html(store_get($$store_subs ??= {}, "$currentOrg", currentOrg).name)}</span> <span class="truncate text-xs">${escape_html(store_get($$store_subs ??= {}, "$currentOrg", currentOrg).plans?.[0]?.planName || "Free")}</span></div> `);
                                Chevrons_up_down($$renderer7, { class: "ml-auto" });
                                $$renderer7.push(`<!---->`);
                              } else {
                                $$renderer7.push("<!--[!-->");
                                $$renderer7.push(`<div class="h-[30px] w-[219px]">`);
                                Skeleton($$renderer7, { class: "h-full w-full" });
                                $$renderer7.push(`<!----></div>`);
                              }
                              $$renderer7.push(`<!--]-->`);
                            },
                    ...
                    Dropdown_menu_content($$renderer5, {
                      ...
                      children: ($$renderer6) => {
                        ...
                        Dropdown_menu_label($$renderer6, {
                          class: "ui:text-muted-foreground ui:text-xs",
                          children: ($$renderer7) => {
                            $$renderer7.push(`<!---->${escape_html(store_get($$store_subs ??= {}, "$t", t)("account.switcher.your_account"))}`);
                          },
                          $$slots: { default: true }
                        });
                        $$renderer6.push(`<!----> <!--[-->`);
                        const each_array_2 = ensure_array_like(accountWorkspaces);
                        for (let $$index_2 = 0, $$length = each_array_2.length; $$index_2 < $$length; $$index_2++) {
                          let org = each_array_2[$$index_2];
                          $$renderer6.push(`<!---->`);
                          Dropdown_menu_item($$renderer6, {
                            ...
                            children: ($$renderer7) => {
                              ...
                              Avatar($$renderer7, {
                                ...
                                children: ($$renderer8) => {
                                  $$renderer8.push(`<!---->`);
                                  Avatar_image($$renderer8, { src: org.avatarUrl, alt: org.name });
                                  $$renderer8.push(`<!----> <!---->`);
                                  Avatar_fallback($$renderer8, {
                                    class: "rounded-lg",
                                    children: ($$renderer9) => {
                                      $$renderer9.push(`<!---->${escape_html(shortenName(org.name))}`);
                                    },
                                    $$slots: { default: true }
                                  });
                                  $$renderer8.push(`<!---->`);
                              ...
                              $$renderer7.push(`<!----> ${escape_html(org.name)}`);
                            },
                            $$slots: { default: true }
                          });
                          $$renderer6.push(`<!---->`);
                        ...
                        if (otherOrgs.length > 0) {
                          ...
                          Dropdown_menu_label($$renderer6, {
                            class: "ui:text-muted-foreground ui:text-xs",
                            children: ($$renderer7) => {
                              $$renderer7.push(`<!---->${escape_html(store_get($$store_subs ??= {}, "$t", t)("account.switcher.other_organizations"))}`);
                            },
                            $$slots: { default: true }
                          });
                          $$renderer6.push(`<!----> <!--[-->`);
                          const each_array_3 = ensure_array_like(otherOrgs);
                          for (let $$index_3 = 0, $$length = each_array_3.length; $$index_3 < $$length; $$index_3++) {
                            let org = each_array_3[$$index_3];
                            $$renderer6.push(`<!---->`);
                            Dropdown_menu_item($$renderer6, {
                              onSelect: () => onClick(org),
                              class: "gap-2 p-2",
                        ...
                        Dropdown_menu_item($$renderer6, {
                          class: "cursor-not-allowed gap-2 p-2 opacity-50",
                          children: ($$renderer7) => {
                            $$renderer7.push(`<div class="flex size-6 items-center justify-center rounded-md border bg-transparent">`);
                            Plus($$renderer7, { class: "size-4" });
                            $$renderer7.push(`<!----></div> <div class="ui:text-muted-foreground ui:font-normal">Add Organization</div> `);
...
function App_breadcrumbs($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    const navItems = getOrgNavigationItems(store_get($$store_subs ??= {}, "$currentOrgPath", currentOrgPath), store_get($$store_subs ??= {}, "$currentOrg", currentOrg), store_get($$store_subs ??= {}, "$isOrgAdmin", isOrgAdmin), store_get($$store_subs ??= {}, "$t", t), page.url.pathname);
    const breadcrumbs = generateBreadcrumbs(page.url.pathname, page.url.search, navItems, store_get($$store_subs ??= {}, "$currentOrgPath", currentOrgPath), store_get($$store_subs ??= {}, "$t", t), page.data);
    let open = false;
          ...
          Breadcrumb_list($$renderer4, {
            ...
            children: ($$renderer5) => {
              ...
              Breadcrumb_item($$renderer5, {
                children: ($$renderer6) => {
                  Org_switcher($$renderer6, { variant: "breadcrumb" });
                },
                $$slots: { default: true }
              });
              $$renderer5.push(`<!----> `);
    ...
    function $$render_inner($$renderer3) {
      ...
      Portal($$renderer3, spread_props([
        portalProps,
        {
          children: ($$renderer4) => {
            $$renderer4.push(`<!---->`);
            Link_preview_content($$renderer4, spread_props([
              {
                "data-slot": "hover-card-content",
                align,
                sideOffset,
                class: cn("ui:bg-popover ui:text-popover-foreground ui:data-[state=open]:animate-in ui:data-[state=closed]:animate-out ui:data-[state=closed]:fade-out-0 ui:data-[state=open]:fade-in-0 ui:data-[state=closed]:zoom-out-95 ui:data-[state=open]:zoom-in-95 ui:data-[side=bottom]:slide-in-from-top-2 ui:data-[side=left]:slide-in-from-end-2 ui:data-[side=right]:slide-in-from-start-2 ui:data-[side=top]:slide-in-from-bottom-2 ui:outline-hidden ui:z-50 ui:mt-3 ui:w-64 ui:rounded-md ui:border ui:p-4 ui:shadow-md ui:outline-none", className)
              },
              restProps,
              {
                get ref() {
...
function Hover_card_trigger($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    function $$render_inner($$renderer3) {
      $$renderer3.push(`<!---->`);
      Link_preview_trigger($$renderer3, spread_props([
        {
          "data-slot": "hover-card-trigger",
          class: cn("ui:cursor-pointer", className)
        },
        restProps,
        {
          get ref() {
...
function App_setup($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    function handleRefresh() {
      if (store_get($$store_subs ??= {}, "$currentOrg", currentOrg).siteName) {
        setupProgressApi.fetchSetupProgress(store_get($$store_subs ??= {}, "$currentOrg", currentOrg).siteName);
      }
    }
    ...
    const progressPercentage = calculateProgress(setupList);
    if (progressPercentage < 100) {
      ...
      Root($$renderer2, {
        ...
        children: ($$renderer3) => {
          $$renderer3.push(`<!---->`);
          Hover_card_trigger($$renderer3, {
            class: "hidden md:block",
            children: ($$renderer4) => {
              Setup_progress($$renderer4, { size: "small", setupItems: setupList });
            },
            $$slots: { default: true }
          });
          $$renderer3.push(`<!----> <!---->`);
          Hover_card_content($$renderer3, {
            class: "hidden w-80 md:block",
            children: ($$renderer4) => {
              if (store_get($$store_subs ??= {}, "$currentOrg", currentOrg).id) {
                $$renderer4.push("<!--[-->");
                $$renderer4.push(`<div class="space-y-2"><div class="flex items-center justify-between"><h4 class="text-sm font-semibold">${escape_html(store_get($$store_subs ??= {}, "$t", t)("setup.your_checklist"))}</h4> `);
                Button($$renderer4, {
                  variant: "secondary",
                  size: "icon",
                  onclick: handleRefresh,
                  disabled: setupProgressApi.isLoading,
                  children: ($$renderer5) => {
                    Refresh_ccw($$renderer5, {
                      class: `size-3 ${stringify(setupProgressApi.isLoading ? "animate-spin" : "")}`
                    });
                  },
                  $$slots: { default: true }
                });
                $$renderer4.push(`<!----></div> <!--[-->`);
                const each_array = ensure_array_like(setupList);
                for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
                  let item = each_array[$$index];
                  $$renderer4.push(`<div class="flex items-center gap-2 text-sm">`);
                  if (item.is_completed) {
                    $$renderer4.push("<!--[-->");
                    Badge_check($$renderer4, { class: "custom size-4 shrink-0 text-green-600! opacity-50" });
                    $$renderer4.push(`<!----> <span class="text-muted-foreground line-through opacity-50">${escape_html(store_get($$store_subs ??= {}, "$t", t)(item.title))}</span>`);
                  } else {
                    $$renderer4.push("<!--[!-->");
                    Circle($$renderer4, { class: "size-4 shrink-0 text-gray-400" });
                    $$renderer4.push(`<!----> <span>${escape_html(store_get($$store_subs ??= {}, "$t", t)(item.title))}</span>`);
                  }
                  $$renderer4.push(`<!--]--></div>`);
                }
                $$renderer4.push(`<!--]--></div>`);
...
function App_header($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<header class="border-border ui:bg-background sticky top-0 z-50 flex h-12 w-full shrink-0 items-center gap-2 border-b transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-8"><div class="flex w-full items-center gap-2 px-4"><!---->`);
    Sidebar_trigger($$renderer2, {});
    $$renderer2.push(`<!----> <div class="h-4 w-2">`);
    Separator($$renderer2, { orientation: "vertical" });
    $$renderer2.push(`<!----></div> `);
    App_breadcrumbs($$renderer2);
    $$renderer2.push(`<!----> <span class="grow"></span> `);
    App_setup($$renderer2);
    $$renderer2.push(`<!----> `);
    Search($$renderer2, {});
    $$renderer2.push(`<!----> <!---->`);
...
function Verify_email_modal($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    const WAIT_SEC = 120;
    ...
    const open = Boolean(!store_get($$store_subs ??= {}, "$profile", profile).isEmailVerified && !!store_get($$store_subs ??= {}, "$profile", profile).id && !!store_get($$store_subs ??= {}, "$currentOrg", currentOrg).id);
    const sendVerificationCode = async () => {
      if (!store_get($$store_subs ??= {}, "$profile", profile).email) {
      ...
      children: ($$renderer3) => {
        ...
        Dialog_content($$renderer3, {
          ...
          children: ($$renderer4) => {
            ...
            Dialog_header($$renderer4, {
              children: ($$renderer5) => {
                ...
                Dialog_title($$renderer5, {
                  children: ($$renderer6) => {
                    $$renderer6.push(`<!---->${escape_html(store_get($$store_subs ??= {}, "$t", t)("verify_email_modal.heading"))}`);
                  },
                  $$slots: { default: true }
                });
                $$renderer5.push(`<!---->`);
            ...
            $$renderer4.push(`<!----> <div class="flex flex-col items-center space-y-6 text-center"><img src="/verify-email.svg" alt="email verification"/> <p class="text-xl">${escape_html(store_get($$store_subs ??= {}, "$t", t)("verify_email_modal.heading"))}</p> <p class="w-[70%] text-sm text-gray-700 dark:text-gray-200">${escape_html(store_get($$store_subs ??= {}, "$t", t)("verify_email_modal.sent_verification"))}
        ${escape_html(store_get($$store_subs ??= {}, "$profile", profile).email)}
        ${escape_html(store_get($$store_subs ??= {}, "$t", t)("verify_email_modal.to_confirm"))}</p> <div class="flex flex-col items-center">`);
            Button($$renderer4, {
              ...
              children: ($$renderer5) => {
                if (loading) {
                  $$renderer5.push("<!--[-->");
                  $$renderer5.push(`${escape_html(store_get($$store_subs ??= {}, "$t", t)("verify_email_modal.loading"))}`);
                } else {
                  $$renderer5.push("<!--[!-->");
                  $$renderer5.push(`${escape_html(store_get($$store_subs ??= {}, "$t", t)("verify_email_modal.resend"))}`);
                }
                $$renderer5.push(`<!--]-->`);
              },
              $$slots: { default: true }
            ...
            if (isSent) {
              $$renderer4.push("<!--[-->");
              $$renderer4.push(`<p class="text-xs text-gray-700">${escape_html(store_get($$store_subs ??= {}, "$t", t)("verify_email_modal.resend_in"))}
            ${escape_html(countDown)}
            ${escape_html(store_get($$store_subs ??= {}, "$t", t)("verify_email_modal.seconds"))}</p>`);
            } else {
              $$renderer4.push("<!--[!-->");
            }
            $$renderer4.push(`<!--]--></div></div>`);
...
function App_logo($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    const plan = store_get($$store_subs ??= {}, "$currentOrg", currentOrg).plans?.[0]?.planName || PLAN.BASIC;
    const utmSource = store_get($$store_subs ??= {}, "$currentOrg", currentOrg).customDomain || (store_get($$store_subs ??= {}, "$currentOrg", currentOrg).siteName ? `${store_get($$store_subs ??= {}, "$currentOrg", currentOrg).siteName}.${TENANT_ROOT_DOMAIN}` : BRAND_ROOT_DOMAIN);
    $$renderer2.push(`<!---->`);
    Sidebar_menu($$renderer2, {
      children: ($$renderer3) => {
        $$renderer3.push(`<!---->`);
            ...
            $$renderer4.push(`<!---->`);
            {
              let child = function($$renderer5, { props }) {
                $$renderer5.push(`<a${attributes({
                  href: `https://${stringify(BRAND_ROOT_DOMAIN)}?utm_source=${stringify(utmSource)}`,
                  target: "_blank",
                  rel: "noopener noreferrer",
                  ...props
                })}><!---->`);
                Avatar($$renderer5, {
                  class: "ui:flex ui:size-6 ui:items-center ui:justify-center",
                  children: ($$renderer6) => {
                    $$renderer6.push(`<!---->`);
                    Avatar_image($$renderer6, { src: "/logo-192.png", alt: "ClassroomIO logo" });
                    $$renderer6.push(`<!---->`);
                  },
                  $$slots: { default: true }
                });
                $$renderer5.push(`<!----> <span class="truncate font-normal">ClassroomIO</span> `);
                Badge($$renderer5, {
                  variant: "outline",
                  class: "capitalize",
                  children: ($$renderer6) => {
                    $$renderer6.push(`<!---->${escape_html(PLAN_NAMES[plan] || plan)}`);
                  },
                  $$slots: { default: true }
                });
                $$renderer5.push(`<!----></a>`);
...
function Nav_main($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    const groups = getOrgNavigationGroups(store_get($$store_subs ??= {}, "$currentOrgPath", currentOrgPath), store_get($$store_subs ??= {}, "$currentOrg", currentOrg), store_get($$store_subs ??= {}, "$isOrgAdmin", isOrgAdmin), store_get($$store_subs ??= {}, "$t", t), page.url.pathname + page.url.search);
    $$renderer2.push(`<!--[-->`);
        ...
        children: ($$renderer3) => {
          if (group.labelKey) {
            ...
            Sidebar_group_label($$renderer3, {
              children: ($$renderer4) => {
                $$renderer4.push(`<!---->${escape_html(store_get($$store_subs ??= {}, "$t", t)(group.labelKey))}`);
              },
              $$slots: { default: true }
            });
            $$renderer3.push(`<!---->`);
                        ...
                        children: ($$renderer6) => {
                          if (item.items) {
                                  ...
                                  let child3 = function($$renderer8, { props: props3 }) {
                                    {
                                      let children = function($$renderer9, isHovered) {
                                        if (item.disabled) {
                                          $$renderer9.push("<!--[-->");
                                          $$renderer9.push(`<span${attributes({
                                            ...props3,
                                            "aria-disabled": "true",
                            ...
                            $$renderer6.push(`<!---->`);
                            {
                              let child2 = function($$renderer7, { props: props2 }) {
                                {
                                  let children = function($$renderer8, isHovered) {
                                    if (item.disabled) {
                                      $$renderer8.push("<!--[-->");
                                      $$renderer8.push(`<span${attributes({
                                        ...props2,
                                        "aria-disabled": "true",
                                        class: "flex cursor-not-allowed items-center gap-2 opacity-50"
                                      })}>`);
                                      if (item.icon) {
                                        ...
                                        const Icon = item.icon;
                                        $$renderer8.push(`<!---->`);
                                        Icon($$renderer8, { isHovered, size: 16, class: "custom" });
                                      ...
                                      $$renderer8.push("<!--[!-->");
                                      $$renderer8.push(`<a${attributes({ href: item.url, ...props2 })}>`);
                                      if (item.icon) {
                                        $$renderer8.push("<!--[-->");
                                        const Icon = item.icon;
                                        $$renderer8.push(`<!---->`);
                                  ...
                                  Hoverable_item($$renderer7, { children });
                                }
                              };
                                ...
                                Sidebar_menu_sub($$renderer7, {
                                  children: ($$renderer8) => {
                                    ...
                                    for (let $$index = 0, $$length3 = each_array_2.length; $$index < $$length3; $$index++) {
                                      let subItem = each_array_2[$$index];
                                      $$renderer8.push(`<!---->`);
                                        ...
                                        children: ($$renderer9) => {
                                          $$renderer9.push(`<!---->`);
                                          {
                                            let child2 = function($$renderer10, { props: props2 }) {
                                              $$renderer10.push(`<a${attributes({ href: subItem.url, ...props2 })}><span>${escape_html(subItem.title)}</span> `);
                                              if (subItem.isPaid && store_get($$store_subs ??= {}, "$isFreePlan", isFreePlan)) {
                                                $$renderer10.push("<!--[-->");
                                                Rocket($$renderer10, { class: "text-primary size-4" });
                                              } else {
                                                $$renderer10.push("<!--[!-->");
                                              }
                                              $$renderer10.push(`<!--]--></a>`);
                                            };
                                            Sidebar_menu_sub_button($$renderer9, { isActive: subItem.isActive, child: child2, $$slots: { child: true } });
                                          }
                                          $$renderer9.push(`<!---->`);
                                        },
                                        $$slots: { default: true }
...
function Upgrade_trigger($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    const sidebar = useSidebar();
    if (store_get($$store_subs ??= {}, "$isFreePlan", isFreePlan)) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<div class="animate-icon">`);
      if (sidebar.open && !sidebar.isMobile) {
        $$renderer2.push("<!--[-->");
        $$renderer2.push(`<div class="animate-gradient-border mx-2 flex flex-col items-center justify-center gap-4 rounded-md px-2 py-6 text-center transition-all ease-in-out">`);
        Rocket($$renderer2, { class: "rocket-launch my-3 size-6", color: "var(--primary)" });
        $$renderer2.push(`<!----> <span class="flex flex-col gap-1"><p class="text-base font-medium">${escape_html(store_get($$store_subs ??= {}, "$t", t)("org_navigation.early_adopter"))}</p> <p class="ui:text-muted-foreground text-xs">${escape_html(store_get($$store_subs ??= {}, "$t", t)("org_navigation.unlock"))}</p></span> `);
        Button($$renderer2, {
          ...
          children: ($$renderer3) => {
            Rocket($$renderer3, { class: "rocket-launch custom size-4" });
            $$renderer3.push(`<!----> ${escape_html(store_get($$store_subs ??= {}, "$t", t)("org_navigation.upgrade"))}`);
          },
          $$slots: { default: true }
        });
        $$renderer2.push(`<!----></div>`);
        ...
        Button($$renderer2, {
          ...
          onclick: openUpgradeModal,
          children: ($$renderer3) => {
            Rocket($$renderer3, { class: "custom size-4" });
            $$renderer3.push(`<!----> <span class="sr-only">${escape_html(store_get($$store_subs ??= {}, "$t", t)("org_navigation.upgrade"))}</span>`);
          },
          $$slots: { default: true }
        });
      }
...
function Org_sidebar($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
...
function Add_org_modal($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    let orgName = "";
    let siteName = generateSitename(orgName);
    let $$settled = true;
    let $$inner_renderer;
    function $$render_inner($$renderer3) {
      $$renderer3.push(`<!---->`);
      Root$1($$renderer3, {
        onOpenChange: (isOpen) => {
          if (!isOpen) store_mutate($$store_subs ??= {}, "$newOrgModal", newOrgModal, store_get($$store_subs ??= {}, "$newOrgModal", newOrgModal).open = false);
        },
        get open() {
          return store_get($$store_subs ??= {}, "$newOrgModal", newOrgModal).open;
        },
        set open($$value) {
          store_mutate($$store_subs ??= {}, "$newOrgModal", newOrgModal, store_get($$store_subs ??= {}, "$newOrgModal", newOrgModal).open = $$value);
          $$settled = false;
        },
        children: ($$renderer4) => {
          $$renderer4.push(`<!---->`);
          Dialog_content($$renderer4, {
            ...
            children: ($$renderer5) => {
              ...
              Dialog_header($$renderer5, {
                children: ($$renderer6) => {
                  ...
                  Dialog_title($$renderer6, {
                    children: ($$renderer7) => {
                      $$renderer7.push(`<!---->${escape_html(store_get($$store_subs ??= {}, "$t", t)("add_org.create_org"))}`);
                    },
                    $$slots: { default: true }
                  });
                  $$renderer6.push(`<!---->`);
              ...
              $$renderer5.push(`<!----> <form class="px-2"><!---->`);
                ...
                children: ($$renderer6) => {
                  ...
                  Field($$renderer6, {
                    children: ($$renderer7) => {
                      ...
                      Field_label($$renderer7, {
                        children: ($$renderer8) => {
                          $$renderer8.push(`<!---->${escape_html(store_get($$store_subs ??= {}, "$t", t)("add_org.name"))}`);
                        },
                        $$slots: { default: true }
                      });
                      $$renderer7.push(`<!----> `);
                      ...
                      if (orgApi.errors.name || orgApi.errors.orgName) {
                        ...
                        Field_error($$renderer7, {
                          children: ($$renderer8) => {
                            $$renderer8.push(`<!---->${escape_html(orgApi.errors.name || orgApi.errors.orgName)}`);
                          },
                          $$slots: { default: true }
                        });
                        $$renderer7.push(`<!---->`);
                  ...
                  Field($$renderer6, {
                    children: ($$renderer7) => {
                      ...
                      Field_label($$renderer7, {
                        children: ($$renderer8) => {
                          $$renderer8.push(`<!---->${escape_html(store_get($$store_subs ??= {}, "$t", t)("add_org.org_sitename"))}`);
                        },
                        $$slots: { default: true }
                      });
                      $$renderer7.push(`<!----> `);
                      Domain_input($$renderer7, {
                        placeholder: "myschool",
                        prefix: "https://",
                        suffix: `.${stringify(TENANT_ROOT_DOMAIN)}`,
                        get value() {
                          return siteName;
                        },
                        set value($$value) {
                      ...
                      if (orgApi.errors.siteName || orgApi.errors.general) {
                        ...
                        Field_error($$renderer7, {
                          children: ($$renderer8) => {
                            $$renderer8.push(`<!---->${escape_html(orgApi.errors.siteName || orgApi.errors.general)}`);
                          },
                          $$slots: { default: true }
                        });
                        $$renderer7.push(`<!---->`);
                  ...
                  Field($$renderer6, {
                    ...
                    children: ($$renderer7) => {
                      Button($$renderer7, {
                        ...
                        class: "ml-auto",
                        children: ($$renderer8) => {
                          $$renderer8.push(`<!---->${escape_html(store_get($$store_subs ??= {}, "$t", t)("add_org.create"))}`);
                        },
                        $$slots: { default: true }
                      });
                    },
...
function _layout($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    {
      $$renderer2.push("<!--[-->");
      Add_org_modal($$renderer2);
    }
    $$renderer2.push(`<!--]--> `);
    Verify_email_modal($$renderer2);
    $$renderer2.push(`<!----> <!---->`);
    Sidebar_provider($$renderer2, {
      children: ($$renderer3) => {
        Org_sidebar($$renderer3);
        $$renderer3.push(`<!----> <!---->`);
        Sidebar_inset($$renderer3, {
          children: ($$renderer4) => {
            App_header($$renderer4);
            $$renderer4.push(`<!----> <div class="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-4 px-4">`);
            if (data.orgName === "*") {
              $$renderer4.push("<!--[-->");
              $$renderer4.push(`<div class="grid auto-rows-min gap-4 md:grid-cols-3">`);
              Skeleton($$renderer4, { class: "aspect-video rounded-xl" });
              $$renderer4.push(`<!----> `);
              Skeleton($$renderer4, { class: "aspect-video rounded-xl" });
              $$renderer4.push(`<!----> `);
              Skeleton($$renderer4, { class: "aspect-video rounded-xl" });
              $$renderer4.push(`<!----></div> `);
              Skeleton($$renderer4, { class: "h-[50vh] w-full rounded-xl" });
              $$renderer4.push(`<!---->`);
            } else {
              $$renderer4.push("<!--[!-->");
              children?.($$renderer4);
...
//# sourceMappingURL=_layout.svelte-CweXmKQq.js.map
