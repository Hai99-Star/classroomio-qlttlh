...
const alertVariants = tv({
  ...
  variants: {
    variant: {
      ...
      destructive: "ui:text-destructive ui:bg-card *:data-[slot=alert-description]:text-destructive/90 ui:[&>svg]:text-current",
      warning: "ui:border-amber-200 ui:bg-amber-50 ui:text-amber-900 ui:dark:border-amber-900 ui:dark:bg-amber-950 ui:dark:text-amber-50 ui:[&>svg]:text-current",
      information: "ui:border-sky-200 ui:bg-sky-50 ui:text-sky-900 ui:dark:border-sky-900 ui:dark:bg-sky-950 ui:dark:text-sky-50 ui:[&>svg]:text-current"
    }
  },
...
function element(node, index, parent, state) {
  ...
  const attributes2 = serializeAttributes(state, node.properties);
  const content = state.all(
    schema.space === "html" && node.tagName === "template" ? node.content : node
  );
  state.schema = schema;
  if (content) selfClosing = false;
  if (attributes2 || !omit || !opening(node, index, parent)) {
    parts.push("<", node.tagName, attributes2 ? " " + attributes2 : "");
      ...
      let normalizePosition = function(p2) {
        if (typeof p2 === "number") {
          if (p2 < 0 || p2 > shiki.source.length)
            throw new ShikiError$1(`Invalid decoration offset: ${p2}. Code length: ${shiki.source.length}`);
          return {
          ...
          if (line === void 0)
            throw new ShikiError$1(`Invalid decoration position ${JSON.stringify(p2)}. Lines length: ${converter.lines.length}`);
          let character = p2.character;
          if (character < 0)
            character = line.length + character;
          if (character < 0 || character > line.length)
            throw new ShikiError$1(`Invalid decoration position ${JSON.stringify(p2)}. Line ${p2.line} length: ${line.length}`);
          return {
            ...p2,
            character,
...
class ShikiError2 extends Error {
  constructor(message) {
    super(message);
    this.name = "ShikiError";
  }
...
const bundledLanguages = {
  bash: () => import('./bash-BzQfOTpL.js'),
  diff: () => import('./diff-Cxl8ody9.js'),
  javascript: () => import('./javascript-C4hZwWz7.js'),
  json: () => import('./json-Z5ntnrap.js'),
  svelte: () => import('./svelte-DTSB1G7G.js'),
  typescript: () => import('./typescript-CUq-p4J9.js')
};
const highlighter = createHighlighterCore({
  themes: [import('./github-light-default-D9xG9AQX.js'), import('./github-dark-default-B0p2K_JS.js')],
  langs: Object.entries(bundledLanguages).map(([_2, lang]) => lang),
...
export { Alert as A, codeVariants as c, highlighter as h };
//# sourceMappingURL=index8-D5Z_9ShZ.js.map
