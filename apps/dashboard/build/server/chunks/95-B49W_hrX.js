...
const load = async ({ params = { slug: "" }, parent, url }) => {
  const { org: parentOrg } = await parent();
  const publicTreeResult = await safeServerApi(
  ...
  if (!org && courseOrgSiteName) {
    const organizationResult = await safeServerApi(
...
//# sourceMappingURL=95-B49W_hrX.js.map
