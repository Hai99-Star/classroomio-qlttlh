...
const cache = {};
function loginActivityDataFromSettled(result) {
  if (result.status === "rejected") return [];
  if (!result.value.ok) return [];
  return result.value.body.data;
}
const load = async ({ params, parent, cookies }) => {
  ...
  if (orgId in cache) {
    const loginActivityResult2 = await safeServerApi(
      () => classroomio.dash["login-activity"].$get({ query: { orgId } }, getApiHeaders(cookies, orgId))
    );
    return {
      orgName: siteName,
      stats: cache[orgId],
      loginActivity: loginActivityResult2.ok ? loginActivityResult2.body.data : []
    };
  }
...
//# sourceMappingURL=55-Dz4qApIv.js.map
