...
function Auth_ui($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    async function signInWithGoogle() {
      ...
      try {
        const apiBaseUrl = public_env.PUBLIC_SERVER_URL?.replace(/\/$/, "");
        const isRenderSplitDomain = apiBaseUrl && window.location.hostname.endsWith(".onrender.com") && new URL(apiBaseUrl, window.location.origin).hostname !== window.location.hostname;
        const isCrossOriginSelfHosted = public_env.PUBLIC_IS_SELFHOSTED === "true" && apiBaseUrl && apiBaseUrl !== window.location.origin && !isRenderSplitDomain;
...
export { Auth_ui as A };
//# sourceMappingURL=auth-ui-CVZ_sluj.js.map
