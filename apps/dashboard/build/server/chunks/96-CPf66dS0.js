...
const load = async ({ params, url }) => {
  ...
  if (inviteToken) {
    const inviteResult = await safeServerApi(
    ...
    const currentOrg2 = {
      ...
      theme: data.organization.theme ?? void 0
    };
    return {
      course: course2,
  ...
  const courseResult = await safeServerApi(
  ...
  const discount = courseData.metadata?.discount ?? 0;
  const showDiscount = courseData.metadata?.showDiscount ?? false;
  ...
  const currentOrg = courseData.org ? {
    ...
    theme: courseData.org.theme ?? void 0
  } : null;
  const course = {
    id: courseData.id,
    ...
    allowNewStudent: courseData.metadata?.allowNewStudent ?? true,
...
//# sourceMappingURL=96-CPf66dS0.js.map
